...
/*
    ...
    * VIEW 옵션들
    - OR REPLACE: 뷰 생성시 기존에 중복된 이름의 뷰가 없다면 새로 뷰를 생성하고, 기존에 중복된 이름의 뷰가 있다면 해당 뷰를 변경(갱신)하는 옵션
...
SELECT * FROM JOB; -- J8 JOB_NAME(NULL)

-- UPDATE
UPDATE VW_JOB SET JOB_NAME='인턴' WHERE JOB_CODE ='J7'; -- 에러
UPDATE VW_JOB SET JOB_CODE = 'J0' WHERE JOB_CODE = 'J8';

-- DELETE
DELETE FROM VW_JOB WHERE JOB_NAME='사원'; -- 에러
...
-- INSERT
INSERT INTO VW_JOB VALUES('인턴'); -- 오류(JOB_CODE가 NOT NULL이기 때문에)
...
INSERT INTO VW_EMP_SAL VALUES(400, '홍길동', 3000000, 36000000); -- 오류

INSERT INTO VW_EMP_SAL(EMP_ID, EMP_NAME, SALARY) VALUES(400, '홍길동', 3000000); -- EMP_NO가 NOT NULL이라 오류남

ALTER TABLE EMPLOYEE MODIFY EMP_NO NULL; -- EMP_NO를 NULL 허용으로 변경하고 나면 오류 안남
SELECT * FROM EMPLOYEE;

-- UPDATE
-- 200번 사원의 연봉을 8000만원으로
UPDATE VW_EMP_SAL SET 연봉 = 80000000 WHERE EMP_ID = 200; -- 오류(산술연산으로 정의된 컬럼은 데이터 변경 불가능)

-- 200번 사원의 급여를 700만원으로
UPDATE VW_EMP_SAL SET SALARY = 7000000 WHERE EMP_ID = 200; -- 성공(산술연산과 무관한 컬럼은 데이터 변경 가능)
...
-- DELETE
DELETE FROM VW_EMP_SAL WHERE 연봉 = 72000000; -- 성공

-- 4. 그룹함수나 GROUP BY 절을 포함한 경우
-- 부서별 급여의 합계, 평균 조회
SELECT DEPT_CODE, SUM(SALARY) "급여 합", FLOOR(AVG(NVL(SALARY,0)))"급여 평균"
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
-- INSERT
INSERT INTO VW_GROUPDEPT VALUES('D3', 8000000, 4000000); -- 오류

-- UPDATE
UPDATE VW_GROUPDEPT SET 합계 = 8000000 WHERE DEPT_CODE = 'D1'; -- 오류

-- DELETE 
DELETE FROM VW_GROUPDEPT WHERE 합계 = 5210000; -- 오류

-- 5. DISTINCT 구문이 포함된 경우(INSERT, UPDATE, DELETE 모두 불가)
...
-- UPDATE
UPDATE VW_DT_JOB SET JOB_CODE = 'J8' WHERE JOB_CODE = 'J7'; -- 에러

-- DELETE
DELETE FROM VW_DT_JOB WHERE JOB_CODE = 'J4'; -- 에러

-- 6. JOIN을 이용해서 여러 테이블을 연결한 경우
-- 사원들의 사번, 사원명, 부서명 조회하는 VIEW 생성
...
-- INSERT
INSERT INTO VW_JOINEMP VALUES(300, '조세오', '총무부'); -- 오류
INSERT INTO VW_JOINEMP(EMP_ID, EMP_NAME) VALUES(500, '조세오'); -- 성공

-- UPDATE
UPDATE VW_JOINEMP SET EMP_NAME = '서동일' WHERE EMP_ID = 200; -- 성공
UPDATE VW_JOINEMP SET DEPT_TITLE = '회계부' WHERE EMP_ID = 200; -- 오류

-- DELETE
DELETE FROM VW_JOINEMP WHERE EMP_ID = '200'; -- 성공
DELETE FROM VW_JOINEMP WHERE DEPT_TITLE = '총무부'; -- 성공
...
-- VIEW 옵션
-- 2. FORCE | NOFORCE
-- NOFORCE(기본값): 서브쿼리에 기술된 테이블이 존재하는 테이블이여야만 뷰가 생성되게 함
DROP TABLE TEST;
...
SELECT * FROM VW_EMP; -- 테이블 생성 후에는 정상적으로 나옴
...
UPDATE VW_EMP SET SALARY = 2000000 WHERE EMP_ID = 200; --> 서브쿼리에 기술한 조건에 부합되지 않기 때문에 변경 불가
