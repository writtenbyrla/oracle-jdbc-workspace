...
-- 부서별 오름차순, 부서 없음이 맨 위에
SELECT NVL(DEPT_CODE, '부서없음') "부서", COUNT(*) "사원 수", COUNT(BONUS) "보너스 받는 사원 수", SUM(SALARY) "총 급여",
        ROUND(AVG(SALARY + SALARY * NVL(BONUS, 0))) "평균 급여", 
        MAX(SALARY) "최고 급여", MIN(SALARY) "최저 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST; -- 1일때는 NULLS FIRST가 작동 안함, 부서 코드를 명시해야 함
...
-- (부서코드, 평균급여)
SELECT DEPT_CODE "부서코드" , TO_CHAR(FLOOR(AVG(NVL(SALARY,0))), 'FM999,999,999') "평균 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 부서 코드가 D5인 사원들의 사번, 사원명, 부서코드, 급여를 조회
...
-- 1. UNION
-- 부서 코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
...
-- 위 쿼리문 대신 WHERE절에 AND 연산자를 사용해서 처리 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND SALARY > 3000000;

-- 4. MINUS
-- 부서 코드인 D5인 사원들 중에서 급여가 300만원 초과인 사원들을 제외해서 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
