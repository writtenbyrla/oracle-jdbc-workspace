 ...
 */
 ...
 /*
    ...
    - SELECT를 통해서 조회된 결과를 RESULT SET이라고 한다.(죽, 조회된 행들의 집합)
    - 조회하고자 하는 컬럼들은 반드시 FROM 절에 기술한 테이블에 존재하는 컬럼이어야 한다.
    - 모든 컬럼을 조회할 경우 컬럼명 대신 * 기호를 사용
 */
 ...
 */
 ...
 SELECT EMP_NAME, SALARY, (SALARY + (BONUS*SALARY)) * 12
 FROM EMPLOYEE;
 ...
 /*
    ...
    - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰따옴표("")로 감싸줘야 한다.
 */
 -- EMPLOYEE 테이블에서 직원명, 연봉, 보너스가 포함된 연봉((급여+(보너스*급여))*12) 조회
 SELECT 
    EMP_NAME AS 직원명,
    SALARY*12 AS "연봉",
    ((SALARY + (BONUS * SALARY))*12) AS "총 소득(원)"
 FROM EMPLOYEE;
...
/*
    ...
    '_': 1글자
    비교대상컬럼 LIKE '_문자' => 비교대상컬럼값에 문자앞에 무조건 한 글자가 올 경우 조회
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사람의 사번, 사원명, 급여, 보너스포함연봉 조회(별칭)
SELECT EMP_ID, EMP_NAME, SALARY, ((SALARY + (BONUS*SALARY))*12) AS "보너스포함연봉"
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%' AND ((SALARY + (BONUS*SALARY))*12) IS NOT NULL;
...
/*
    ...
    - NULLS FIRST: 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치(생략시 DESC일때 기본값)
    - NULLS LAST: 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 매치(생략시 ASC일때 기본값)
*/
