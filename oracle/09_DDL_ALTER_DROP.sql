...
/*
    1_2) 컬럼 수정(MODIFY)
         ...
         - 기본값 변경: ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 변경할 기본값;
                        --> DEFAULT NULL로 변경시 : DEFAULT 삭제
*/
-- DEPT_ID 컬럼의 데이터 타입을 CHAR(3)으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; -- 값이 있을 때 오류 발생(VARCHAR2 -> NUMBER)
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); 
--> 변경하려는 자료형의 크기보다 이미 큰 값이 존재해서 에러 (VARCHAR2 -> VARCHAR2)

ALTER TABLE DEPT_COPY MODIFY CNAME NUMBER;
--> 값이 없으면 데이터타입 변경 가능(VARCHAR2 -> NUMBER)
...
ALTER TABLE DEPT_COPY
    ...
    MODIFY LNAME DEFAULT '미국'; -- 기존 데이터의 값은 유지, 테이블의 기본값만 변경됨
...
/*
    ...
    - 참조되고 있는 컬럼이 있다면 삭제 불가능(ROLLBACK 불가)
    - 테이블에는 최소 한개의 컬럼은 존재해야 함
*/
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_ID;
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_TITLE;
ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID;
ALTER TABLE DEPT_COPY DROP COLUMN CNAME;
ALTER TABLE DEPT_COPY DROP COLUMN LNAME; -- 테이블에 최소 1개의 컬럼은 존재해야 함, 삭제 불가

ROLLBACK; -- 컬럼 삭제에서 ROLLBACK해도 복구 안됨
...
-- 만약 삭제하고자 한다면
-- 방법1. 자식테이블을 먼저 삭제한 후 부모테이블을 삭제하는 방법(순서 중요)
DROP TABLE MEMBER;
DROP TABLE MEMBER_GRADE;
