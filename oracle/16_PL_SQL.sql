...
-- 각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY컬럼과
...
-- 일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 
-- NCODE 값이 KO일 경우 => TEAM에 '국내팀' 대입, 그게 아닐 경우 => TEAM에 '해외팀' 대입
-- 사번(EID), 이름(ENAME), 부서명(DTITLE), 소속(TEAM)에 대해 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    DTITLE DEPARTMENT.DEPT_TITLE%TYPE;
...
/*
    1-3) IF ~ ELSIF ~ ELSE 구문
         ...
         END IF;
*/
-- 사용자에게 점수를 입력받아 SCORE 변수(데이터타입: NUMBER)에 저장한 후 학점은 입력된 점수에 따라 GRADE 변수(데이터타입: CHAR(1))에 저장
...
-- DEPT_CODE에 따라서 알맞는 부서를 출력 (DNAME 데이터 타입은 VARCHAR2(30))
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8인 경우 기술지원부,
-- D9인 경우 총무부, 나머지는 부서없음
...
/*
    ...
    [표현식]
    LOOP 
        반복적으로 실행할 구문;(춤추는 민소, 술먹는 미리, 누워있는 지은)
        
        * 반목문을 빠져나갈 수 있는 구문(못나감)
    END LOOP;
...
-- UNIQUE 제약조건 위배시
BEGIN
    UPDATE EMPLOYEE SET EMP_ID = '203' WHERE EMP_NAME = '&이름';
EXCEPTION
...
-- TOO_MANY_ROWS: SELECT한 결과가 여러 행인 경우(한 행이 리턴되어야 하는 경우)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
