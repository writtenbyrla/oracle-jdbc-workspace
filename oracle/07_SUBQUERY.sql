...
FROM EMPLOYEE
WHERE SALARY IN (8000000, 3900000, 2550000, 3760000, 3660000, 2890000, 2490000);
...
-- 사수에 해당하는 사번을 조회
SELECT DISTINCT MANAGER_ID 사수사번
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID 사수사번
                 FROM EMPLOYEE
                 WHERE MANAGER_ID IS NOT NULL)
UNION
-- 사원 조회
...
FROM EMPLOYEE
WHERE EMP_ID NOT IN (SELECT DISTINCT MANAGER_ID 사수사번
                     FROM EMPLOYEE
                     WHERE MANAGER_ID IS NOT NULL);
                    
-- SELECT 절에서 서브쿼리를 사용하는 방법
SELECT EMP_ID, EMP_NAME, DEPT_CODE,
    CASE WHEN EMP_ID IN (SELECT DISTINCT MANAGER_ID 사수사번
                         FROM EMPLOYEE
                         WHERE MANAGER_ID IS NOT NULL)
         THEN '사수'
         ELSE '사원'
...
-- 1) 인라인 뷰를 사용하는 방법
SELECT DEPT_CODE "부서코드", ROUND(AVG(NVL(SALARY,0))) "평균 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY AVG(NVL(SALARY,0)) DESC;

SELECT ROWNUM, "부서코드", "평균 급여" -- 별칭 이용하지 않으면 오류남
FROM (SELECT DEPT_CODE "부서코드", ROUND(AVG(NVL(SALARY,0))) "평균 급여"
      FROM EMPLOYEE
      GROUP BY DEPT_CODE
